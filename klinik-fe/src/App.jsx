...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  html, body { font-family: var(--font); background: var(--bg); color: var(--text-1); -webkit-font-smoothing: antialiased; }
  ...
  .sb-logo-icon { width:40px; height:40px; background:var(--primary); border-radius:var(--r-sm); display:flex; align-items:center; justify-content:center; font-size:20px; flex-shrink:0; }
  .sb-logo-name { font-size:15px; font-weight:700; color:#fff; line-height:1.2; }
  .sb-logo-role { font-size:11px; color:rgba(255,255,255,0.32); margin-top:2px; }

  .sb-nav { padding:10px 10px; flex:1; overflow-y:auto; }
  .sb-section { font-size:10px; font-weight:700; letter-spacing:0.1em; color:rgba(255,255,255,0.22); text-transform:uppercase; padding:10px 10px 5px; }
  .nav-btn {
    ...
    text-align:left; transition:all var(--tr); margin-bottom:2px;
  }
  ...
  .sb-footer { padding:14px 16px 18px; border-top:1px solid rgba(255,255,255,0.07); flex-shrink:0; }
  ...
  .sb-name  { font-size:13px; font-weight:600; color:#fff; line-height:1.3; }
  .sb-email { font-size:11px; color:rgba(255,255,255,0.32); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:148px; }
  .btn-logout { width:100%; padding:9px; background:rgba(239,68,68,0.1); color:#fca5a5; border:1px solid rgba(239,68,68,0.2); border-radius:var(--r-sm); cursor:pointer; font-size:13px; font-weight:600; transition:all var(--tr); font-family:var(--font); }
  ...
  .topbar {
    ...
    padding:0 32px; position:sticky; top:0; z-index:100; box-shadow:var(--sh-sm);
    flex-shrink:0;
  }
  .topbar-l { display:flex; align-items:center; gap:12px; min-width:0; }
  .topbar-title { font-size:17px; font-weight:700; color:var(--text-1); white-space:nowrap; }
  .topbar-sub   { font-size:12px; color:var(--text-3); white-space:nowrap; }
  .hamburger { display:none; width:36px; height:36px; border:1.5px solid var(--border); background:var(--surface-2); border-radius:var(--r-sm); cursor:pointer; font-size:18px; align-items:center; justify-content:center; flex-shrink:0; }

  .page-body { padding:28px 32px; flex:1; width:100%; max-width:1300px; }
  ...
  /* ── BUTTONS ── */
  .btn { display:inline-flex; align-items:center; gap:6px; padding:9px 18px; border-radius:var(--r-sm); font-size:14px; font-weight:600; cursor:pointer; border:none; transition:all var(--tr); white-space:nowrap; font-family:var(--font); }
  ...
  .lbl { display:block; font-size:13px; font-weight:600; color:var(--text-2); margin-bottom:6px; }
  .lbl span { color:var(--danger); }
  .inp { width:100%; padding:10px 14px; border:1.5px solid var(--border); border-radius:var(--r-sm); font-size:14px; color:var(--text-1); background:var(--surface); transition:border-color var(--tr); display:block; }
  ...
  th { padding:12px 16px; text-align:left; font-size:11px; font-weight:700; color:var(--text-3); text-transform:uppercase; letter-spacing:0.07em; white-space:nowrap; }
  td { padding:13px 16px; color:var(--text-1); font-size:14px; vertical-align:middle; border-bottom:1px solid var(--surface-2); }
  ...
  .modal-box { background:var(--surface); border-radius:var(--r-xl); width:100%; max-width:520px; max-height:92vh; display:flex; flex-direction:column; box-shadow:var(--sh-lg); animation:scaleIn .22s ease; }
  ...
  .stat-card { background:var(--surface); border-radius:var(--r-lg); border:1px solid var(--border); padding:24px; box-shadow:var(--sh-sm); position:relative; overflow:hidden; }
  ...
  .stat-lbl  { font-size:13px; color:var(--text-3); margin-bottom:6px; }
  .stat-val  { font-size:38px; font-weight:800; color:var(--text-1); line-height:1; }

  /* ── DOCTOR CARDS ── */
  .doc-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:20px; }
  .doc-card { background:var(--surface); border-radius:var(--r-lg); border:1.5px solid var(--border); padding:24px; box-shadow:var(--sh-sm); display:flex; flex-direction:column; transition:all var(--tr); }
  .doc-card:hover { border-color:var(--primary); box-shadow:var(--sh-md); transform:translateY(-2px); }
  .doc-ava { width:60px; height:60px; border-radius:var(--r-md); background:linear-gradient(135deg,var(--primary),var(--primary-mid)); display:flex; align-items:center; justify-content:center; font-size:28px; margin-bottom:16px; }

  /* ── SHORTCUT ── */
  .shortcut-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(180px,1fr)); gap:16px; margin-bottom:32px; }
  .shortcut-card { background:var(--surface); border-radius:var(--r-lg); border:1.5px solid var(--border); padding:24px; cursor:pointer; text-align:left; transition:all var(--tr); box-shadow:var(--sh-sm); font-family:var(--font); }
  .shortcut-card:hover { border-color:var(--primary); background:var(--primary-light); transform:translateY(-2px); box-shadow:var(--sh-md); }

  /* ── ALERTS / INFO ── */
  .alert { padding:11px 14px; border-radius:var(--r-sm); font-size:13px; font-weight:500; margin-bottom:16px; }
  ...
  /* ── TOAST ── */
  .toast { position:fixed; bottom:24px; right:24px; z-index:9999; padding:13px 20px; border-radius:var(--r-md); font-size:14px; font-weight:600; max-width:340px; box-shadow:var(--sh-lg); animation:fadeUp .3s ease; cursor:pointer; }
  ...
  .ph h2 { font-size:22px; font-weight:800; color:var(--text-1); margin-bottom:4px; }
  .ph p  { font-size:14px; color:var(--text-3); }

  /* ── EMPTY ── */
  .empty { text-align:center; padding:56px 24px; color:var(--text-3); }
  .empty-ic { font-size:48px; margin-bottom:14px; opacity:0.55; }
  ...
  .loading-c { display:flex; align-items:center; justify-content:center; gap:12px; padding:52px; color:var(--text-3); font-size:14px; }
  .grid-2    { display:grid; grid-template-columns:1fr 1fr; gap:14px; }
  .detail-row { display:grid; grid-template-columns:130px 1fr; gap:10px; align-items:start; margin-bottom:14px; }
  .detail-k   { font-size:11px; font-weight:700; color:var(--text-3); text-transform:uppercase; letter-spacing:0.06em; padding-top:3px; }
  .detail-v   { font-size:14px; color:var(--text-1); line-height:1.65; }
  .rekam-ok { background:#ecfdf5; border-radius:var(--r-md); border:1px solid #a7f3d0; padding:14px; margin-top:16px; }
  .info-box  { background:var(--primary-light); border-radius:var(--r-sm); border:1px solid #a7d8d3; padding:10px 14px; font-size:13px; color:var(--primary-dark); margin-bottom:16px; }
  .fade-up   { animation: fadeUp .32s ease both; }
  .search-wrap { position:relative; max-width:380px; margin-bottom:24px; }
  .search-ico  { position:absolute; left:13px; top:50%; transform:translateY(-50%); font-size:15px; color:var(--text-3); pointer-events:none; }
  ...
  .auth-card { background:var(--surface); border-radius:var(--r-xl); padding:44px 40px; width:100%; max-width:430px; box-shadow:var(--sh-lg); position:relative; z-index:1; animation:fadeUp .38s ease; }
  .auth-logo { text-align:center; margin-bottom:34px; }
  .auth-logo-ic { width:68px; height:68px; background:linear-gradient(135deg,var(--primary),var(--primary-mid)); border-radius:var(--r-lg); display:flex; align-items:center; justify-content:center; font-size:32px; margin:0 auto 16px; box-shadow:0 8px 32px rgba(10,124,110,0.28); }
  .auth-logo h1 { font-size:26px; font-weight:800; color:var(--text-1); margin-bottom:6px; }
  .auth-logo p  { font-size:14px; color:var(--text-3); }
  .auth-btn { width:100%; padding:13px; margin-top:8px; background:var(--primary); color:#fff; border:none; border-radius:var(--r-md); font-size:15px; font-weight:700; cursor:pointer; transition:all var(--tr); font-family:var(--font); }
  ...
  .auth-footer { text-align:center; margin-top:22px; font-size:14px; color:var(--text-3); }
...
function Empty({ icon = "📋", title, sub }) {
  return <div className="empty"><div className="empty-ic">{icon}</div><div className="empty-t">{title}</div>{sub && <div className="empty-s">{sub}</div>}</div>;
}
...
function AdminHome({ call, user }) {
  const [s, setS] = useState({ d:0, j:0, r:0 });
  useEffect(() => {
    Promise.all([call("/dokters"), call("/jadwals"), call("/rekam-medis",{},true)])
      .then(([d,j,r]) => setS({ d:d.data.length, j:j.data.length, r:r.data.length }))
      .catch(() => {});
  }, []);
  return (
    <div className="fade-up">
      <div style={{ marginBottom:28 }}>
        <h2 style={{ fontSize:24, fontWeight:800, marginBottom:6 }}>Selamat datang, {user.name} 👋</h2>
        <p style={{ color:"var(--text-3)", fontSize:14 }}>Panel Administrasi Klinik Sehat</p>
      </div>
      <div className="stat-grid" style={{ marginBottom:28 }}>
        {[["👨‍⚕️","Total Dokter",s.d],["📅","Total Jadwal",s.j],["📋","Rekam Medis",s.r]].map(([ic,lb,vl]) => (
      ...
      <div className="card card-p">
        <h3 style={{ fontSize:14, fontWeight:700, marginBottom:12 }}>🔑 Auth Headers yang Dikirim Otomatis</h3>
        <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
          <div className="alert a-info" style={{ margin:0, flex:1, minWidth:200 }}><strong>Authorization:</strong> Bearer &lt;JWT&gt;</div>
          <div className="alert a-info" style={{ margin:0, flex:1, minWidth:200 }}><strong>X-API-KEY:</strong> &lt;api_key&gt;</div>
        </div>
      </div>
    </div>
  );
...
function DokterMgmt({ call }) {
  ...
  const load = async () => { setLoading(true); try { const r = await call("/dokters"); setData(r.data); } finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
  const s = k => e => setForm(p => ({ ...p, [k]:e.target.value }));
  const save = async e => {
    e.preventDefault(); setSaving(true);
    try {
      modal==="add" ? await call("/dokters",{method:"POST",body:JSON.stringify(form)},true)
                   : await call(`/dokters/${form.id}`,{method:"PUT",body:JSON.stringify(form)},true);
      await load(); setModal(null);
    } finally { setSaving(false); }
  };
  const del = async id => { if (!confirm("Hapus dokter ini?")) return; await call(`/dokters/${id}`,{method:"DELETE"},true); await load(); };
  return (
    <div className="fade-up">
...
function JadwalMgmt({ call }) {
  const [data, setData] = useState([]); const [dokters, setDokters] = useState([]);
  ...
  const load = async () => {
    setLoading(true);
    try { const [j,d] = await Promise.all([call("/jadwals"),call("/dokters")]); setData(j.data); setDokters(d.data); }
  ...
  const save = async e => {
    e.preventDefault(); setSaving(true);
    try {
      modal==="add" ? await call("/jadwals",{method:"POST",body:JSON.stringify(form)},true)
                   : await call(`/jadwals/${form.id}`,{method:"PUT",body:JSON.stringify(form)},true);
      await load(); setModal(null);
    } finally { setSaving(false); }
  };
  const del = async id => { if (!confirm("Hapus jadwal?")) return; await call(`/jadwals/${id}`,{method:"DELETE"},true); await load(); };
  return (
    <div className="fade-up">
        ...
        <Modal title={modal==="add"?"Tambah Jadwal":"Edit Jadwal"} onClose={()=>setModal(null)}>
          <form onSubmit={save}>
            ...
            <div className="grid-2">
              ...
              <Field label="Status"><Sel value={form.is_aktif} onChange={e=>setForm(p=>({...p,is_aktif:Number(e.target.value)}))}>
...
function RekamMedisMgmt({ call }) {
  ...
  const load = async () => {
    setLoading(true);
    try { const [r,d] = await Promise.all([call("/rekam-medis",{},true),call("/dokters")]); setData(r.data); setDokters(d.data); }
    finally { setLoading(false); }
  };
  useEffect(()=>{load();},[]);
  const s = k => e => setForm(p=>({...p,[k]:e.target.value}));
  const save = async e => {
    e.preventDefault(); setSaving(true);
    try {
      modal==="add" ? await call("/rekam-medis",{method:"POST",body:JSON.stringify(form)},true)
                   : await call(`/rekam-medis/${form.id}`,{method:"PUT",body:JSON.stringify(form)},true);
      await load(); setModal(null);
    } finally { setSaving(false); }
  };
  const del = async id => { if (!confirm("Hapus rekam medis?")) return; await call(`/rekam-medis/${id}`,{method:"DELETE"},true); await load(); };
  return (
    <div className="fade-up">
      <PH title="Rekam Medis" sub="Kelola rekam medis seluruh pasien"
        action={<button className="btn btn-primary" onClick={()=>{setForm({reservasi_id:"",user_id:"",dokter_id:"",tanggal_periksa:"",diagnosis:"",resep_obat:"",catatan_dokter:""});setModal("add");}}>+ Tambah</button>}/>
      <div className="card">
        {loading ? <Loading/> : <Table
          cols={[
            ...
            {key:"diagnosis",label:"Diagnosis",render:v=><span className="truncate" title={v}>{v?.length>45?v.slice(0,45)+"…":v}</span>},
          ]}
          data={data}
          actions={row => <>
      ...
      {modal && (
        <Modal title={modal==="add"?"Tambah Rekam Medis":"Edit Rekam Medis"} onClose={()=>setModal(null)}>
          <form onSubmit={save}>
            <div className="info-box">💡 Masukkan ID dari data yang ada di database.</div>
            <div className="grid-2">
              <Field label="ID Reservasi" req><Inp type="number" value={form.reservasi_id||""} onChange={s("reservasi_id")} required/></Field>
              <Field label="ID User (Pasien)" req><Inp type="number" value={form.user_id||""} onChange={s("user_id")} required/></Field>
            </div>
            <Field label="Dokter" req>
              <Sel value={form.dokter_id||""} onChange={s("dokter_id")} required>
                <option value="">Pilih dokter...</option>
                {dokters.map(d=><option key={d.id} value={d.id}>{d.nama}</option>)}
              </Sel>
            </Field>
...
function PasienHome({ user, call, setSection }) {
  ...
  useEffect(()=>{ call("/reservasis").then(r=>setReservasis(r.data.slice(0,4))).catch(()=>{}); },[]);
  const shortcuts = [
  ...
  return (
    <div className="fade-up">
      <div style={{marginBottom:28}}>
        <h2 style={{fontSize:24,fontWeight:800,marginBottom:6}}>Halo, {user.name} 👋</h2>
        <p style={{color:"var(--text-3)",fontSize:14}}>Selamat datang di Portal Pasien Klinik Sehat</p>
      </div>
      <div className="shortcut-grid">
        {shortcuts.map(s=>(
          <button key={s.label} className="shortcut-card" onClick={()=>setSection(s.sec)}>
            <div style={{fontSize:30,marginBottom:12}}>{s.icon}</div>
            <div style={{fontWeight:700,fontSize:14,marginBottom:4,color:"var(--text-1)"}}>{s.label}</div>
      ...
      {reservasis.length > 0 && (
        <>
          <h3 style={{fontSize:15,fontWeight:700,marginBottom:14}}>Reservasi Terbaru</h3>
          <div className="card">
            <Table
              cols={[
...
function DokterList({ call, onReservasi }) {
  ...
  useEffect(()=>{ call("/dokters").then(r=>setData(r.data)).finally(()=>setLoading(false)); },[]);
  ...
  return (
    <div className="fade-up">
      ...
      <div className="search-wrap">
        ...
        <Inp className="inp search-inp" style={{paddingLeft:40}} placeholder="Cari nama atau spesialisasi..." value={search} onChange={e=>setSearch(e.target.value)}/>
      </div>
      {loading ? <Loading/> : (
        <div className="doc-grid">
          {filtered.map(d=>(
            <div key={d.id} className="doc-card">
              ...
              <h3 style={{fontSize:16,fontWeight:700,marginBottom:4}}>{d.nama}</h3>
              <p style={{fontSize:13,color:"var(--primary)",fontWeight:700,marginBottom:6}}>{d.spesialisasi}</p>
              <p style={{fontSize:12,color:"var(--text-3)",marginBottom:4}}>STR: {d.no_str}</p>
              <p style={{fontSize:15,fontWeight:800,marginBottom:d.bio?14:20}}>Rp {Number(d.biaya_konsultasi).toLocaleString("id-ID")}</p>
              {d.bio && <p style={{fontSize:13,color:"var(--text-3)",lineHeight:1.6,flexGrow:1,marginBottom:18}}>{d.bio}</p>}
...
function ReservasiForm({ call, showToast, initialDokter, onDone }) {
  const [dokters, setDokters] = useState([]); const [jadwals, setJadwals] = useState([]);
  const [form, setForm] = useState({dokter_id:initialDokter?.id||"",jadwal_id:"",tanggal_reservasi:"",keluhan:""});
  const [loading, setLoading] = useState(false);
  const today = new Date().toISOString().split("T")[0];
  const s = k => e => setForm(p=>({...p,[k]:e.target.value}));
  useEffect(()=>{ call("/dokters").then(r=>setDokters(r.data)); },[]);
  useEffect(()=>{
    if (!form.dokter_id) { setJadwals([]); return; }
    call("/jadwals").then(r=>{ setJadwals(r.data.filter(j=>String(j.dokter_id)===String(form.dokter_id)&&j.is_aktif)); setForm(p=>({...p,jadwal_id:""})); });
  },[form.dokter_id]);
  const submit = async e => {
    e.preventDefault(); setLoading(true);
    try { await call("/reservasis",{method:"POST",body:JSON.stringify(form)}); showToast("Reservasi berhasil dibuat! 🎉"); onDone(); }
    catch(err) { showToast(err.message||"Gagal membuat reservasi","error"); }
    finally { setLoading(false); }
  };
  return (
    <div className="fade-up">
      ...
      <div className="card" style={{maxWidth:560}}>
        <div className="card-p">
          <form onSubmit={submit}>
            ...
            <Field label="Keluhan" req><Txta value={form.keluhan} onChange={s("keluhan")} placeholder="Ceritakan keluhan Anda..." required style={{minHeight:110}}/></Field>
...
function MyReservasi({ call, showToast }) {
  ...
  const load = async () => { setLoading(true); try { const r=await call("/reservasis"); setData(r.data); } finally { setLoading(false); } };
  useEffect(()=>{load();},[]);
  const batalkan = async id => {
    if (!confirm("Batalkan reservasi ini?")) return;
    try { await call(`/reservasis/${id}`,{method:"DELETE"}); showToast("Reservasi dibatalkan."); await load(); }
    ...
    <div className="fade-up">
      ...
      <div className="card">
        {loading ? <Loading/> : <Table
          ...
          data={data}
          actions={row => <>
            <button className="btn btn-outline btn-sm" onClick={async()=>{try{const r=await call(`/reservasis/${row.id}`);setDetail(r.data);}catch{setDetail(row);}}}>Detail</button>
        ...
        <Modal title="Detail Reservasi" onClose={()=>setDetail(null)}>
          ...
          {detail.rekam_medis && (
            <div className="rekam-ok">
              <p style={{fontWeight:700,color:"#065f46",marginBottom:8,fontSize:13}}>✅ Rekam Medis Tersedia</p>
              <p style={{fontSize:13,color:"#065f46",marginBottom:4}}><strong>Diagnosis:</strong> {detail.rekam_medis.diagnosis}</p>
              <p style={{fontSize:13,color:"#065f46"}}><strong>Resep:</strong> {detail.rekam_medis.resep_obat}</p>
            </div>
          )}
...
function MyRekamMedis({ call }) {
  ...
  useEffect(()=>{
    call("/reservasis").then(async r=>{
      ...
      setData(details.filter(d=>d.status==="fulfilled"&&d.value?.data?.rekam_medis).map(d=>({...d.value.data.rekam_medis,dokter:d.value.data.dokter})));
...
// ─── DASHBOARD SHELL ──────────────────────────────────────────────────────────
const ADMIN_NAV  = [["home","🏠","Beranda"],["dokters","👨‍⚕️","Dokter"],["jadwals","📅","Jadwal"],["rekam-medis","📋","Rekam Medis"]];
const PASIEN_NAV = [["home","🏠","Beranda"],["cari-dokter","👨‍⚕️","Cari Dokter"],["reservasi","📅","Buat Reservasi"],["my-reservasi","📋","Reservasi Saya"],["rekam-medis-pasien","🏥","Rekam Medis Saya"]];
const PAGE_TITLES = { home:"Beranda", dokters:"Manajemen Dokter", jadwals:"Manajemen Jadwal", "rekam-medis":"Rekam Medis", "cari-dokter":"Cari Dokter", reservasi:"Buat Reservasi", "my-reservasi":"Reservasi Saya", "rekam-medis-pasien":"Rekam Medis Saya" };
    ...
    <div className="app-shell">
      ...
      <aside className={`sidebar ${open?"open":""}`}>
        ...
        <div className="sb-footer">
          ...
          <div style={{marginBottom:8}}><Badge label={user.role}/></div>
...
export default function App() {
  ...
  const callApi = useCallback((path,opts={},needKey=false)=>
    api(path,opts,token||null,needKey?(apiKey||null):null),
  [token,apiKey]);
